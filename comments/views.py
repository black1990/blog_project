...
def article_comment(request,article_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 将评论和被评论的文章关联起来。
            comment.artilce = article_object
            ...
            # 因此我们传了三个模板变量给 single.html，
            # 一个是文章（Article），一个是评论列表，一个是表单 form
            # 注意这里我们用到了 artcile_object.comment_set.all() 方法，
            # 这个用法有点类似于 Post.objects.all()
            # 其作用是获取这篇 post 下的的全部评论，
            # 因为 Post 和 Comment 是 ForeignKey 关联的，
            # 因此使用 post.comment_set.all() 反向查询全部评论。
            # 具体请看下面的讲解。
            comment_list = article_object.comment_set.all()
            context = {'article': article_object,
                       'form': form,
                       'comment_list': comment_list
                       }
            return render(request, 'blog/single.html.html', context=context)
