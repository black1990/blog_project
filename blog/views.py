...
#     '''
#     获取用户请求页的页码。我们给页码设置的 URL 类似于 http://zmrenwu.com/?page=2。
#     其中 ? 号后面的 page=2 表示用户请求的页码数。
#     Django 会将问号后面的请求参数保存到 request.GET 属性里，这是一个类字典的属性。
#     例如这里 page 作为键被保存，其值为 2。
#     '''
...
#         article_list=paginator.page(1)#如果请求的页数不是数字返回第一页
#     except EmptyPage:
#         article_list=paginator.page(paginator.num_pages)  #如果请求页数超过总数返回总数
...
def achives(request,year,month):
     article_achive_list=Article.objects.filter(create_time__year=year,create_time__month=month)
     return  render(request,'blog/achives.html',{'article_achive_list':article_achive_list})
#分类查询
def categorys(request,category_id):
    #cate=Category.objects.get(pk=category_id)
    cate=get_object_or_404(Category,pk=category_id)
    category_filter=Article.objects.filter(category=cate)
    return render(request,'blog/category.html',{'category_article_list':category_filter})
#标签云查询
